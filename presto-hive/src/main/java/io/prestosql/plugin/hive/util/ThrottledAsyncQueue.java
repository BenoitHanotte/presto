        ...
        extends AsyncQueue<T>
{
    ...
    public synchronized <O> ListenableFuture<O> borrowBatchAsync(int maxSize, Function<List<T>, BorrowResult<T, O>> function)
    {
        ...
        3.  In order to be able to properly test this method, we need to make sure that if we wait for the result of a
            ...
            This allows to test by making sure that non-throttled calls to this method return a future for
        ...
        to complete.
         */
        this.throttleSignal = thenAsync(throttleSignal, Bucket::next, throttleService);

        /*
        Once the future above completes and the queue is not empty anymore, we can then query for a new batch.
        If the queue is empty, wait for the notEmptySignal.
        ...
        final ListenableFuture<O> batchFuture = thenAsync(
                ...
                bucket -> {
                    synchronized (this) {
                        if (maxSize > 0 && size() == 0) {
                            // if the queue is empty we wait for it to become non-empty again before we can query a batch
                            // otherwise we would query an empty batch.
                            return thenAsync(
                                    notEmptySignal,
                                    any -> super.borrowBatchAsync(bucket.atMost(maxSize), function),
                                    directExecutor());
                        }
                        else {
                            return super.borrowBatchAsync(bucket.atMost(maxSize), function);
                        }
                    }
                },
                executor);
    ...
    private class Bucket
    {
        ...
        private int atMost(int maxSize)
        {
            ...
            return Math.min(max, remainingSize.getAndUpdate(v -> v - Math.min(max, v)));
        }
